...
models_path=filepath[0:filepath.rfind('\\')+1]+'train.h5'
CHRS = string.ascii_lowercase + string.digits
cnn=load_model(models_path)
...
def mainCode(image):
    ...
    if table!=0:
        ...
        result=''.join([CHRS[predictions[i]] for i in range(5)])
        print("图片识别结果为:%s"%(result))
        # K.clear_session()
        return result
    else:
        print('image split failure')
        return 0